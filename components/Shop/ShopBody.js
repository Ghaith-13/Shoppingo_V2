...
const ShopBody = ({
  ...
  const handleChange = (data) => {
    ...
    setProductsDisplayed(
      products.slice(
        data.selected * productsPerPage,
        data.selected * productsPerPage + productsPerPage
      )
    );
  };
    ...
    >
      ...
      <div className="flex flex-col-reverse md:items-center md:flex-row md:justify-between">
        ...
        <Search
          ...
          shopRoute={shopRoute}
          setSelectedOptionsForFemale={setSelectedOptionsForFemale}
          setSelectedOptionsForMale={setSelectedOptionsForMale}
        />
        {/* Classification */}
        <div className="flex flex-col-reverse md:flex-row md:space-x-3 md:items-center">
          ...
          <div className="relative z-10 flex flex-col group">
            <button
              className={`flex justify-between items-center py-2 px-3 ${
                WShow ? 'bg-[#bb0202] dark:bg-gradient-to-tl' : ''
              } mt-5 md:mt-0`}
              onClick={() => {
                setWShow((prev) => !prev);
                setMShow(false);
            ...
            <TiArrowSortedDown
              className={` absolute top-[50px] left-[8px] md:top-[30px] md:left-[30px] text-[#bb0202] dark:text-[#474747] dark:md:text-[#323232] w-7 h-7 ${
                WShow ? 'flex' : 'hidden'
              } `}
            />
            ...
            <Classification
              ...
              uniqueProducts={uniqueProducts}
              shopRoute={shopRoute}
              selectedOptionsForMale={selectedOptionsForMale}
              selectedOptionsForFemale={selectedOptionsForFemale}
              setSelectedOptionsForFemale={setSelectedOptionsForFemale}
              setSelectedOptionsForMale={setSelectedOptionsForMale}
            />
          </div>
          ...
          <div className="relative z-10 flex flex-col group">
            <button
              className={`flex justify-between items-center py-2 px-3 ${
                MShow ? 'bg-[#bb0202] dark:bg-gradient-to-tl' : ''
              } mt-5 md:mt-0`}
              onClick={() => {
                setMShow((prev) => !prev);
                setWShow(false);
          ...
          <div className="relative z-10 flex flex-col group">
            <button
              className={`flex justify-between items-center py-2 px-3 ${
                PShow ? 'bg-[#bb0202] dark:bg-gradient-to-tl' : ''
              }  mt-5 md:mt-0`}
              onClick={() => {
                setPShow((prev) => !prev);
                setWShow(false);
      ...
      {role == 'seller' && !shopIdRoute && (
        <div className="mb-7 flex space-x-3 justify-between items-center">
          <button
            ...
            onClick={() => router.push('/profile/sellerDashboard/order')}
          >
            طلباتي
          </button>
          ...
          <button
            className="w-[100px] py-1"
            onClick={() => router.push('/profile/sellerDashboard/addProduct')}
          >
            إضافة منتج
          </button>
        </div>
          ...
          {productsDisplayed?.map((one, index) => {
            if (shopRoute) {
              return (
                <DynamicProduct
                  key={index}
                  brandId={one.brand._id}
                  id={one.shippestProduct._id}
                  img={one.shippestProduct.frontImgURL}
                  fav={false}
                  offer={one.shippestProduct.price !== one.price}
                  oPrice={one.shippestProduct.price}
                  nPrice={one.price}
                  rating={String(one.meanRating)}
                  setNoteMsg={setNoteMsg}
                  uniqueProducts={uniqueProducts}
                  shopRoute={shopRoute}
                />
              ...
              return (
                <DynamicProduct
                  key={index}
                  brandId={one.product.brand}
                  id={one.product._id}
                  img={one.product.frontImgURL}
                  fav={false}
                  offer={one.product.price !== one.updatedPrice}
                  oPrice={one.product.price}
                  nPrice={one.updatedPrice}
                  rating={String(one.meanRating)}
                  setNoteMsg={setNoteMsg}
                  uniqueProducts={uniqueProducts}
                  shopRoute={shopRoute}
